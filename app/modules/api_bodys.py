...
def body_generator(data_to_yield,title60,imagenes,dataUser,listing_type_id):
    WARRANTY_TIME = dataUser.loc[0,'warranty_time']
    if 'sin' in WARRANTY_TIME:
        WARRANTY_TIME = '0'
    meli_currency = dataUser.loc[0,'meli_currency']
    video_youtube = dataUser.loc[0,'video_youtube']
    channels = dataUser.loc[0,'channels']
    if channels == 1:
    ...
    body = {
        "title": title60, #OK
        "category_id": data_to_yield['id_meli_category'],  #  #OK
        "price": int(data_to_yield['meli_price']),  #OK
        "currency_id": str(meli_currency),  #OK
        "available_quantity": int(data_to_yield['available_quantity']), #OK 
        "buying_mode": "buy_it_now", #OK
        "condition": "new",  #OK
        "listing_type_id": listing_type_id, #OK PENDIENTE!! - Clásica gold_special  #gold_pro Premium
        "seller_custom_field": data_to_yield['sku'], #OK
        "video_id": video_youtube, #Video Presentacion Just Market # Fijar
        "channels": channels,
        "sale_terms": [
        ...
        "attributes": data_to_yield['meli_attributes'],  #OK
    }

    if WARRANTY_TIME != ('0' or '0 meses' or '0 mes' or '0 dias'):
        wtDict = {
                "id": "WARRANTY_TYPE",
                "value_name": "Garantía del vendedor"   #OK FIjar
            }
        body['sale_terms'].append(wtDict)
        wtDict = {
                "id": "WARRANTY_TIME",
                "value_name": WARRANTY_TIME,  
            }
    ...
    if data_to_yield['manufacturing_time'] != '0 dias':
        
        mtDict = {
                "id": "MANUFACTURING_TIME",
                "value_name": data_to_yield['manufacturing_time'], #OK
        }
        body['sale_terms'].append(mtDict)
